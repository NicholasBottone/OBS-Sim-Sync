--[[
	OBS Lua script
    Syncs FTC Freight Frenzy Sim game states with OBS text sources (for up to 4 independent servers)
    By Nicholas Bottone
--]]
...
local function checkFile(id)
    ...
    do
        ...
        -- Auto Red
        local f = io.open(dataDirectory.."/R_Auto.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Auto Blue
        local f = io.open(dataDirectory.."/B_Auto.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Tele Red
        local f = io.open(dataDirectory.."/R_Tele.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Tele Blue
        local f = io.open(dataDirectory.."/B_Tele.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- End Red
        local f = io.open(dataDirectory.."/R_End.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- End Blue
        local f = io.open(dataDirectory.."/B_End.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
...
function script_description()
	return "Syncs FTC Freight Frenzy sim game state to OBS text sources"
end
