--[[
	OBS Lua script
    Syncs FTC Ultimate Goal Sim game states with OBS text sources (for up to 4 independent servers)
    By Nicholas Bottone
--]]
...
local function checkFile(id)
    ...
    do
        ...
        -- RED SCORE
        local f = io.open(dataDirectory.."/R_Score.txt", "rb")
        local redScore = "0"
        if f then
            settings = obs.obs_data_create()
        ...
        -- BLUE SCORE
        local f = io.open(dataDirectory.."/B_Score.txt", "rb")
        local blueScore = "0"
        if f then
            settings = obs.obs_data_create()
        ...
        -- Auto Red
        local f = io.open(dataDirectory.."/R_S_Auto.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Auto Blue
        local f = io.open(dataDirectory.."/B_S_Auto.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Tele Red
        local f = io.open(dataDirectory.."/R_S_Tele.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Tele Blue
        local f = io.open(dataDirectory.."/B_S_Tele.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- End Red
        local f = io.open(dataDirectory.."/R_S_End.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- End Blue
        local f = io.open(dataDirectory.."/B_S_End.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Rings Red
        local f = io.open(dataDirectory.."/R_C_RINGS.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
            source = obs.obs_get_source_by_name("Red Rings"..sourceStr)
            obs.obs_source_update(source, settings)
            obs.obs_data_release(settings)
            obs.obs_source_release(source)
            f:close()
        ...
        -- Rings Blue
        local f = io.open(dataDirectory.."/B_C_RINGS.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
            source = obs.obs_get_source_by_name("Blue Rings"..sourceStr)
            obs.obs_source_update(source, settings)
            obs.obs_data_release(settings)
            obs.obs_source_release(source)
            f:close()
        ...
        -- Penalty Red
        local f = io.open(dataDirectory.."/R_Pen.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
        ...
        -- Penalty Blue
        local f = io.open(dataDirectory.."/B_Pen.txt", "rb")
        if f then
            settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", safetyRead(f))
...
function script_description()
	return "Syncs FTC Ultimate Goal sim game state to OBS text sources"
end
